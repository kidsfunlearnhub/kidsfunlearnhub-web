// --- 1. Base Digit Paths (0-9) ---
// These are perfectly drawn base digits. 
const baseDigits = {
...
// This safely shifts coordinates left or right to place two numbers side-by-side
...
// Generate the full 1 to 40 array automatically!
const numberData = [];
for (let i = 1; i <= 40; i++) {
    ...
    if (strNum.length === 1) {
        // Center single digits (1-9)
        finalPaths = baseDigits[strNum].map(p => shiftPath(p, 50));
    } else {
        // Place double digits side-by-side (10-40)
        let tensDigit = strNum[0];
        let onesDigit = strNum[1];
        finalPaths = [
            ...baseDigits[tensDigit].map(p => shiftPath(p, 10)), // Shift left
            ...baseDigits[onesDigit].map(p => shiftPath(p, 90))  // Shift right
        ];
    }
    ...
    numberData.push({
        number: strNum,
        paths: finalPaths,
        audioFile: `${i}.mp3` // Looks for 1.mp3, 12.mp3, 40.mp3, etc.
    });
}
...
async function openCard(cardElement, itemData) {
    ...
    currentAudio = new Audio(`sounds/en/numbers/${itemData.audioFile}`);
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const completed = await animateSingleStroke(lines[i], pencilGroup, 1000, cardElement); // Sped up slightly to 1000ms for numbers
        if (!completed) return; 
