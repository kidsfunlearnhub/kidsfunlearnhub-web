...
window.onload = function() {
    ...
    let themeIndex = parseInt(sessionStorage.getItem('puzzleShapeThemeIndex')) || 0;
    ...
    let currentLang = sessionStorage.getItem('puzzleShapeLang') || 'en'; 
    let score = parseInt(sessionStorage.getItem('puzzleShapeScore')) || 0;
    ...
    const uiDict = {
        "game-title": { en: "🟢 Shape Picture Puzzle!", hi: "🟢 आकार चित्र पहेली!", mr: "🟢 आकार चित्र कोडे!" },
        ...
        "page-title": { en: "Shape Picture Puzzle Game | KidsFunLearnHub", hi: "आकार चित्र पहेली खेल | KidsFunLearnHub", mr: "आकार चित्र कोडे खेळ | KidsFunLearnHub" }
    };
    ...
    function updateLanguage(lang) {
        currentLang = lang;
        sessionStorage.setItem('puzzleShapeLang', lang); 
    ...
    function playInstructionAudio() {
        let instructionAudio = new Audio(`sounds/${currentLang}/shapes/${currentTargetShape}.mp3`);
    ...
    function startNewRound() {
        ...
        currentTargetShape = allShapes[Math.floor(Math.random() * allShapes.length)];
        
        // Note: Using images/shapes/basic/ per the user's finding setup
        const imgUrl = `images/shapes/basic/${currentTargetShape}.webp`;
    ...
    function handleSlotClick(slot) {
        ...
        if (slot.dataset.index === selectedPieceCard.dataset.piece) {
    ...
    function showRoundComplete() {
        ...
        const triggerPhaseTwo = () => {
            feedbackText.innerText = shapeDict[currentTargetShape][currentLang];
            // Pointing to the basic shape folder
            feedbackImg.src = `images/shapes/basic/${currentTargetShape}.webp`;
            feedbackImg.classList.remove("hidden"); 

            let shapeNameAudio = new Audio(`sounds/${currentLang}/shapes/${currentTargetShape}.mp3`);
            ...
            const advanceToNext = () => {
                ...
                if (roundsPlayedThisSession >= ROUNDS_BEFORE_RELOAD) {
                    sessionStorage.setItem('puzzleShapeScore', score);
                    sessionStorage.setItem('puzzleShapeLang', currentLang);
                    let nextThemeIndex = (themeIndex + 1) % themes.length;
                    sessionStorage.setItem('puzzleShapeThemeIndex', nextThemeIndex);
    ...
    document.getElementById("backBtn").addEventListener("click", () => {
        sessionStorage.removeItem('puzzleShapeScore'); 
        sessionStorage.removeItem('puzzleShapeThemeIndex'); 
